...
const Navbar = () => {
  ...
  return (
    <div className={`h-full w-full p-10 ${theme === "dark" ? "dark-bg " : "bg-white"}`}>

      {/* Top Section */}
      <div className="flex items-center gap-2 pt-6">
        <img src={logo} alt="Logo" />
        <h2 className={`text-lg font-normal ${theme === "dark" ? "dark-text" : "text-[#352E5B]"}`}>
      ...
      <ul className="mt-12 space-y-9">
        {[
          ...
          <div key={index} className="flex items-center gap-6 cursor-pointer  p-2 rounded-lg">
            <img src={item.img} alt={item.text} className="h-5 w-5" />
